    ...
    const form = useForm<z.infer<typeof formSchema>>({
     ...
     useEffect(() => {
        if (session?.user?.name) {
            ...
            form.clearErrors('author_name'); // Очищаем ошибки валидации
        }
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) { 
        ...
        try {
            ...
            form.reset( {
               rating: 5,
                comment: "",
                author_name: username, // Сохраняем username после reset
            });
            router.refresh();
        } catch (error) {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel></FormLabel>
              <FormControl>
                <Input  {...field}  value={username || field.value}  disabled={true}  className="cursor-not-allowed"/> 
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
